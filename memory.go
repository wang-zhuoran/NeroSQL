...
type MemoryBackend struct {
	...
	models map[string]*model // new field to store machine learning models
}

func (mb *MemoryBackend) CreateModel(cms *CreateModelStatement) error {
	/*
	   type model struct {
	   	name         string
	   	features     []string
	...
	*/
	model := createModel()
	model.name = cms.Name.Value
	model.modelType = cms.ModelType.Value
	model.args = make([]string, len(cms.Args))
	for i, arg := range cms.Args {
		model.args[i] = arg.Value
	}
	model.tableName = cms.Table.Value

	data, ok := mb.tables[model.tableName]
	if !ok {
	...
	// Fill the variable data with the content in table
	model.data = data.rows

	if model.modelType == "knn" {
		model.features = model.args[:len(model.args)-3]
		model.labels = model.args[len(model.args)-3]

	} else if model.modelType == "linear_regression" {
		model.features = model.args[:len(model.args)-1]
		model.labels = model.args[len(model.args)-1]
	}

	mb.models[model.name] = model //将model存入model list
	return nil
}

func (mb *MemoryBackend) TrainModel(tms *TrainModelStatement) (TrainResult, error) {
	model, ok := mb.models[tms.Name.Value]
	if !ok {
		return "error", ErrModelDoesNotExist
	}

	// Train the model
	if model.modelType == "knn" {
		// Convert the k value from string to integer
		k, err := strconv.Atoi(model.args[len(model.args)-2])
		if err != nil {
			return "error", ErrModelArgsException
		}
		// Get the distance metric from the model arguments
		distanceMetric := model.args[len(model.args)-1]

		// Get the labels and features from the model arguments
		model.labels = model.args[len(model.args)-3]
		model.features = model.args[:len(model.args)-3]

		// Convert the features to a slice of slices of interfaces
		var featuresInterfaceSlice [][]interface{}
		for _, feature := range model.features {
			featuresInterfaceSlice = append(featuresInterfaceSlice, []interface{}{feature})
		}

		// Convert the labels to a slice of integers
		labels := make([]int, len(model.labels))
		for i, label := range model.labels {
			intLabel, err := strconv.Atoi(string(label))
			if err != nil {
				// handle error
				fmt.Println("Error converting the labels to int:", err)

			}
			labels[i] = intLabel
		}

		// Define the knn_model function using the knn algorithm
		knn_model := func(x []interface{}) int {
			return knn(featuresInterfaceSlice, labels, k, distanceMetric)
		}

		// Save the trained model to a file
		var encodedModel bytes.Buffer
		encoder := gob.NewEncoder(&encodedModel)
		err = encoder.Encode(knn_model)
		if err != nil {
			fmt.Println("Error encoding model:", err)

		}

		// Save the encoded model to a file
		err = ioutil.WriteFile("models/"+model.name+".gob", encodedModel.Bytes(), 0644)
		if err != nil {
	...
	// mb.models[tms.Name.Value] = model
	return "TrainResult", nil
}

func (mb *MemoryBackend) Predict(prds *PredictStatement) (PredResult, error) {
	model, ok := mb.models[prds.Name.Value]
	if !ok {
		return "error", ErrModelDoesNotExist
	}
	...
	if model.modelType == "knn" {
		...
		// Read the encoded model from a file
		encodedModel, err := ioutil.ReadFile("models/" + model.name + ".gob")
		if err != nil {
		...
		// Decode the model
		var decodedModel func([]interface{}) int
		decoder := gob.NewDecoder(bytes.NewReader(encodedModel))
		err = decoder.Decode(&decodedModel)
		if err != nil {
			fmt.Println("Error decoding model:", err)
			// return
		}

		// Convert the prds.Input to a slice of interfaces
		inputInterfaceSlice := make([]interface{}, len(prds.Input))
		for i, input := range prds.Input {
			inputInterfaceSlice[i] = input
		}

		// Predict using the model
		prediction := decodedModel(inputInterfaceSlice)
		str := strconv.Itoa(prediction)
		return PredResult(str), nil
