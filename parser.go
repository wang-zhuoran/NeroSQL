...
func (p Parser) parseTokenList(tokens []*Token, initialCursor uint, endToken Token) ([]*Token, uint, bool) {
	cursor := initialCursor
	var result []*Token
	// var ok bool

	for {
		token, newCursor, ok := p.parseToken(tokens, cursor, endToken)
		if !ok {
			break
		}
		...
		commaToken, newCursor, ok := p.parseToken(tokens, cursor, tokenFromSymbol(CommaSymbol))
		if !ok {
			break
		}
		cursor = newCursor
...
func (p Parser) parseCreateModelStatement(tokens []*Token, initialCursor uint, _ Token) (*CreateModelStatement, uint, bool) {
	cursor := initialCursor
	ok := false

	_, cursor, ok = p.parseToken(tokens, cursor, tokenFromKeyword(CreateModelKeyword))
	if !ok {
		return nil, initialCursor, false
	}
	...
	name, newCursor, ok := p.parseTokenKind(tokens, cursor, IdentifierKind)
	if !ok {
		p.helpMessage(tokens, cursor, "Expected model name")
		return nil, initialCursor, false
	}
	cursor = newCursor

	_, cursor, ok = p.parseToken(tokens, cursor, tokenFromKeyword(AsKeyword))
	if !ok {
		p.helpMessage(tokens, cursor, "Expected AS keyword")
		return nil, initialCursor, false
	}

	modelType, newCursor, ok := p.parseTokenKind(tokens, cursor, IdentifierKind)
	if !ok {
		p.helpMessage(tokens, cursor, "Expected model type")
		return nil, initialCursor, false
	}
	cursor = newCursor

	args, newCursor, ok := p.parseTokenList(tokens, cursor, tokenFromKeyword(AsKeyword))
	if !ok {
		p.helpMessage(tokens, cursor, "Expected model arguments")
		return nil, initialCursor, false
	}
	cursor = newCursor
	...
	predOutput, newCursor, ok := p.parseTokenKind(tokens, cursor, IdentifierKind)
	if !ok {
		p.helpMessage(tokens, cursor, "Expected prediction output")
		return nil, initialCursor, false
	}
	cursor = newCursor

	_, cursor, ok = p.parseToken(tokens, cursor, tokenFromKeyword(FromKeyword))
	if !ok {
		p.helpMessage(tokens, cursor, "Expected FROM keyword")
		return nil, initialCursor, false
	}
...
func (p Parser) parseTrainModelStatement(tokens []*Token, initialCursor uint, _ Token) (*TrainModelStatement, uint, bool) {
	cursor := initialCursor
	ok := false

	_, cursor, ok = p.parseToken(tokens, cursor, tokenFromKeyword(TrainModelKeyword))
	if !ok {
		return nil, initialCursor, false
	}
	...
	_, cursor, ok = p.parseToken(tokens, cursor, tokenFromKeyword(WithKeyword))
	if !ok {
		p.helpMessage(tokens, cursor, "Expected WITH keyword")
		return nil, initialCursor, false
	}

	args, newCursor, ok := p.parseTokenList(tokens, cursor, tokenFromSymbol(SemicolonSymbol))
	if !ok {
		p.helpMessage(tokens, cursor, "Expected model arguments")
		return nil, initialCursor, false
	}
	...
	return &TrainModelStatement{
		Name: *name,
		Args: args,
	}, cursor, true
}

func (p Parser) parsePredictStatement(tokens []*Token, initialCursor uint, _ Token) (*PredictStatement, uint, bool) {
	cursor := initialCursor
	ok := false
	...
	input, newCursor, ok := p.parseTokenList(tokens, cursor, tokenFromSymbol(RightParenSymbol))
	if !ok {
		p.helpMessage(tokens, cursor, "Expected input arguments")
		return nil, initialCursor, false
	}
	cursor = newCursor
...
func (p Parser) parseStatement(tokens []*Token, initialCursor uint, _ Token) (*Statement, uint, bool) {
	...
	crtMdl, newCursor, ok := p.parseCreateModelStatement(tokens, cursor, semicolonToken)
	if ok {
		return &Statement{
			Kind:                 CreateModelKind,
			CreateModelStatement: crtMdl,
		}, newCursor, true
	}

	trnMdl, newCursor, ok := p.parseTrainModelStatement(tokens, cursor, semicolonToken)
	if ok {
		return &Statement{
			Kind:                TrainModelKind,
			TrainModelStatement: trnMdl,
		}, newCursor, true
	}

	pred, newCursor, ok := p.parsePredictStatement(tokens, cursor, semicolonToken)
	if ok {
		return &Statement{
			Kind:             PredictKind,
			PredictStatement: pred,
		}, newCursor, true
	}
