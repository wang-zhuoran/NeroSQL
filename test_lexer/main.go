...
func lexCreateModel(source string, ic cursor) (*Token, cursor, bool) {
	...
	cur.pointer += uint(len(CreateModelKeyword) + 1)
	cur.loc.Col += uint(len(CreateModelKeyword) + 1)
	...
	// extract table name
	tableName, newCursor, ok := lexIdentifier(source, cur)
	if !ok {
		return nil, ic, false
	}
	cur = newCursor
	...
	return &Token{
		Value: string(CreateModelKeyword) + " " + modelName.Value + " as " + modelType.Value + args.Value + " from " + tableName.Value,
		Kind:  KeywordKind,
		Loc:   ic.loc,
	}, cur, true
}
...
func lex(source string) ([]*Token, error) {
	var tokens []*Token
	cur := cursor{}
	if nerosql.IsAIOperation(source) {
		tokens, err := tokenize(source)
		if err == nil {
			return tokens, nil
		} else {
			return nil, fmt.Errorf("error with AI operation")
		}
	} else {
