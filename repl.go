...
func RunRepl(b Backend) {
	...
	fmt.Println("Welcome to NeroSQL.")
	// sendMessage(conn, "Welcome to NeroSQL.")
repl:
	for {
		fmt.Print("> ")
		line, err := l.Readline()
		...
		for _, stmt := range ast.Statements {
			...
			case CreateModelKind:
				err := b.CreateModel(stmt.CreateModelStatement)
				if err != nil {
					fmt.Println("Error creating the model:", err)
				}
			case TrainModelKind:
				res, err := b.TrainModel(stmt.TrainModelStatement)
				if err != nil {
				...
				fmt.Println(res)
			case PredictKind:
				res, err := b.Predict(stmt.PredictStatement)
				if err != nil {
					fmt.Println("Error predicting the results:", err)
					continue repl
				}
				fmt.Println(res)
			}
